...
function Page() {
  ...
  useEffect(() => { supabase.from("transactions").select("*").order("created_at", { ascending: false }).limit(100).then(({ data }) => setRows(data ?? [])); }, []);
  const cols = [{key:"user_id",label:"User"},{key:"type",label:"Type"},{key:"amount_usd",label:"Amount"},{key:"status",label:"Status"},{key:"created_at",label:"Date"}];
  return (<><PageHeader title="Transactions" /><Panel>
