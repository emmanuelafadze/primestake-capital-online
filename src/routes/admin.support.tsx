...
function Page() {
  ...
  useEffect(() => { supabase.from("support_messages").select("*").order("created_at", { ascending: false }).limit(100).then(({ data }) => setRows(data ?? [])); }, []);
  const cols = [{key:"user_id",label:"User"},{key:"sender",label:"From"},{key:"body",label:"Message"},{key:"created_at",label:"Date"}];
  return (<><PageHeader title="Support Inbox" /><Panel>
