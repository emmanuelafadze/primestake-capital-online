...
function Page() {
  ...
  useEffect(() => { supabase.from("purchases").select("*").order("created_at", { ascending: false }).limit(100).then(({ data }) => setRows(data ?? [])); }, []);
  const cols = [{key:"user_id",label:"User"},{key:"amount_usd",label:"Amount"},{key:"status",label:"Status"},{key:"paystack_reference",label:"Reference"},{key:"created_at",label:"Date"}];
  return (<><PageHeader title="Purchases" /><Panel>
...
export const Route = createFileRoute("/admin/purchases")({ component: Page });
