...
function Page() {
  ...
  useEffect(() => { supabase.from("withdrawals").select("*").order("created_at", { ascending: false }).limit(100).then(({ data }) => setRows(data ?? [])); }, []);
  const cols = [{key:"user_id",label:"User"},{key:"amount_usd",label:"Amount"},{key:"status",label:"Status"},{key:"destination",label:"Destination"},{key:"created_at",label:"Requested"}];
  return (<><PageHeader title="Withdrawals" /><Panel>
